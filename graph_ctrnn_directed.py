...
#  CONFIG
# --------------------------------------------------------------------------- #
DATA_DIR = 'C:/Users/alexg/Onedrive/Escritorio/phd/folder_save/rnn_aux/directed/'  # Alex
...
def directed_channels(ei, ej):
    """Build directed (src -> dst) channels: both i<-j and j<-i.
    Channel c feeds dst[c], reading src[c].  Length = 2E."""
    src = np.concatenate([ej, ei])   # i<-j reads j ; j<-i reads i
    dst = np.concatenate([ei, ej])
    return src.astype(np.int64), dst.astype(np.int64)


# --------------------------------------------------------------------------- #
#  Integrator (directed, diagonal-free)
# --------------------------------------------------------------------------- #
@njit(cache=True, fastmath=True)
def _simulate(q0, up0, um0, src, dst, ei, ej, zedge, K, wqu, wuq,
              tau_q, tau_u, noise, steps, dt, rq, rup, rum, rec_every):
    N = q0.shape[0]
    C = src.shape[0]          # directed channels = 2E
    E = ei.shape[0]
    q = q0.copy(); up = up0.copy(); um = um0.copy()
    sdt = np.sqrt(dt)
    nrec = steps // rec_every
    Q = np.empty((nrec, N)); UP = np.empty((nrec, C))
    ridx = 0
    for t in range(steps):
        # ---- directed auxiliaries: read ONLY the source node ----
        upn = up.copy(); umn = um.copy()
        for c in range(C):
            s_node = src[c]
            a = wqu*(2.0*q[s_node]-1.0)
            sp = 1.0/(1.0+np.exp(-(a + zedge[c])))
            sm = 1.0/(1.0+np.exp(-(a - zedge[c])))
            upn[c] = up[c] + dt*(sp - up[c])/tau_u
            umn[c] = um[c] + dt*(sm - um[c])/tau_u
            if noise > 0.0:
                upn[c] += noise*sdt*rup[t, c]
                umn[c] += noise*sdt*rum[t, c]
            if upn[c] < 1e-6: upn[c] = 1e-6
            elif upn[c] > 1.0-1e-6: upn[c] = 1.0-1e-6
            if umn[c] < 1e-6: umn[c] = 1e-6
            elif umn[c] > 1.0-1e-6: umn[c] = 1.0-1e-6
        # ---- q update: fixed K (neighbour) + directed aux (writes dst) ----
        inp = np.zeros(N)
        for k in range(E):
            i = ei[k]; j = ej[k]
            inp[i] += K*(2.0*q[j]-1.0)
            inp[j] += K*(2.0*q[i]-1.0)
        for c in range(C):
            inp[dst[c]] += wuq*((2.0*up[c]-1.0) + (2.0*um[c]-1.0))
        qnew = q.copy()
        for i in range(N):
        ...
        if (t % rec_every) == 0 and ridx < nrec:
            ...
            for c in range(C): UP[ridx, c] = up[c]
            ridx += 1
    return q, up, um, Q, UP
...
def simulate(ei, ej, N, z, K=0.2, wqu=1.5, wuq=1.0, tau_q=5.0, tau_u=0.5,
    ...
    zedge = np.full(C, float(z))
    a0 = wqu*(2*q0[src]-1)
    up0 = sig(a0 + zedge); um0 = sig(a0 - zedge)
    if noise > 0:
        rq = rng.standard_normal((steps, N))
        rup = rng.standard_normal((steps, C)); rum = rng.standard_normal((steps, C))
    else:
        rq = np.zeros((steps, N)); rup = np.zeros((steps, C)); rum = np.zeros((steps, C))
    return _simulate(q0.astype(np.float64), up0.astype(np.float64),
                     um0.astype(np.float64), src, dst, ei, ej,
                     zedge.astype(np.float64), float(K), float(wqu), float(wuq),
                     float(tau_q), float(tau_u), float(noise),
                     steps, float(dt), rq, rup, rum, int(rec_every))
...
def keff_matrix(ei, ej, N, z, wqu, wuq, K):
    """Effective q-q connectivity at q=1/2 (directed; diagonal ~ 0)."""
    eps = 1e-4
    M = np.zeros((N, N))
    q0 = np.full(N, 0.5)
    src, dst = directed_channels(ei, ej)
    ...
    def q_input(q):
        inp = np.zeros(N)
        a = wqu*(2*q[src]-1)                       # directed: read src only
        up = 1.0/(1.0+np.exp(-(a + z)))
        um = 1.0/(1.0+np.exp(-(a - z)))
        aux = wuq*((2*up-1) + (2*um-1))
        np.add.at(inp, dst, aux)                   # write dst only
        np.add.at(inp, ei, K*(2*q[ej]-1))          # fixed K
        np.add.at(inp, ej, K*(2*q[ei]-1))
        return inp
...
# --------------------------------------------------------------------------- #
#  Critical-z helpers (DIAGONAL-FREE -> single factor)
# --------------------------------------------------------------------------- #
def solve_z(K, d, W):
    """Two roots of sigma'(z_c)=A via the logit form, diagonal-free:
       criticality (d/4)[2K + 8 W sigma'] = 1  =>  sigma'(z_c) = (2/d - K)/(4 W).
    """
    A = (2/d - K) / (4*W)               # <-- diagonal-free: 4 W (was 8 W)
    disc = 1 - 4*A
    if disc < 0:
    ...
    return logit(s1), logit(s2)


def zc_directed(K, d, W):
    """z_c = 2 arccosh( sqrt( d W / (2 - K d) ) ), diagonal-free."""
    denom = 2 - K*d
    if denom <= 0:
        return np.nan
    arg = d*W/denom
    if arg < 1:
        return np.nan
    return 2*np.arccosh(np.sqrt(arg))


def plot_critical_z_vs_W_K(W_list=np.arange(0.0, 1.0, 1e-3),
                           K_list=np.arange(0.0, 0.6, 1e-3), d=4, z=0):
    """3-panel: regime | z_c | effective connectivity, all over (K, W).
    DIAGONAL-FREE coupling:  J_edge(z) = 2K + 8 W sigma'(z).
    Criticality (d/4) J_edge(z_c) = 1  ->  arg = d W / (2 - K d)."""
    W, K = np.meshgrid(W_list, K_list, indexing='ij')
    denom = 2 - K*d
    with np.errstate(divide='ignore', invalid='ignore'):
        arg = d*W/denom                         # <-- diagonal-free (was 2 d W)
    ...
    # effective connectivity per edge at this z:  J_edge = 2K + 8 W sigma'(z)
    Jeff = 2*K + 8*W*sig(z)*(1-sig(z))

    extent = [K_list.min(), K_list.max(), W_list.min(), W_list.max()]  # x=K, y=W
    ...
    im0 = ax[0].imshow(regime, aspect='auto', extent=extent, origin='lower',
                       cmap=cmap, norm=norm)
    ax[0].set_title('Regimes', fontsize=13)
    im1 = ax[1].imshow(zc, aspect='auto', extent=extent, origin='lower')
    ax[1].set_title(r'Critical stimulus $z_c$', fontsize=13)
    im2 = ax[2].imshow(Jeff, aspect='auto', extent=extent, origin='lower',
                       cmap='magma')
    ax[2].contour(K, W, regime, levels=[0.5, 1.5], colors='cyan', linewidths=1.2)
    ax[2].set_title(f'Effective connectivity (z={z})', fontsize=13)

    for a in ax:
        a.set_xlabel('Fixed coupling, K')
    ...
    fig.colorbar(im2, ax=ax[2], fraction=0.046,
                 label=r'$J_{eff}(z{=}$' + str(z) + r'$)=2K+8W\sigma^\prime$')
    fig.tight_layout()
    for term in ['.png', '.svg']:
...
if __name__ == "__main__":
    ...
    # ---- aux-count / diagonal info ----
    src, dst = directed_channels(ei, ej)
    print(f"edges={len(ei)}, directed channels={len(src)} (4 aux per edge "
          f"counting +/-), each q (deg {d}) receives from {2*d}, sends to {2*d}")
    ...
    for col, (tag, Q) in enumerate(ex.items()):
        ...
        for i in range(N):
            a.plot(t, Q[:, i], lw=0.8, alpha=0.7,
                   color="#c0392b" if Q[-1, i] > 0.5 else "#2471a3")
        a.set_title(tag, fontsize=14)
        a.set_xlabel("Time"); a.set_ylabel("q_i")
        a.set_ylim(-0.04, 1.04); a.spines[["top", "right"]].set_visible(False)
    fig.tight_layout()
    fig.savefig(os.path.join(DATA_DIR, "pitchfork_and_dynamics.png"),
                dpi=300, bbox_inches="tight")
    fig.savefig(os.path.join(DATA_DIR, "pitchfork_and_dynamics.svg"),
                dpi=130, bbox_inches="tight")
    print("saved pitchfork_and_dynamics")
    ...
    for k, (z, M) in enumerate(zip(z_mats, mats)):
        ...
        ax[k].set_title(f"z={z}\nmean edge K_eff={M[A>0].mean():.3f}", fontsize=13)
        ax[k].set_xlabel("q_j")
        if k == 0: ax[k].set_ylabel("q_i")
        fig.colorbar(im, ax=ax[k], fraction=0.046)
    fig.tight_layout()
    fig.savefig(os.path.join(DATA_DIR, "keff_vs_z.png"), dpi=300, bbox_inches="tight")
    fig.savefig(os.path.join(DATA_DIR, "keff_vs_z.svg"), dpi=300, bbox_inches="tight")
    print("saved keff_vs_z")
    ...
    print(f"max |diag(K_eff)| at z=0 = {np.abs(np.diag(mats[0])).max():.2e} (should be ~0)")
